...
object ChurnLabeller{
  def main(args: Array[String]) : Unit ={
    var first_argument = args(0);
    var second_argument = args(1);
    var third_argument = args(2);
    ...
    val data = spark
      ...
      .csv("/home/achraf/kaggle_challenge/project/data/transactions_final.csv")

    val historyCutoff = first_argument;
    println(historyCutoff)

    val historyData = data.filter(col("transaction_date")>="20150101" and col("transaction_date")<=lit(historyCutoff))
    ...
    val predictionCandidates = userExpire
      .filter(
        col("last_expire") >= second_argument and col("last_expire") <= third_argument 
      )
    ...
    resultSet.repartition(1).write.option("header",value=true).csv("sample_user_label" + "_" + historyCutoff)
    println("New scala code " + historyCutoff)
    Thread.sleep(10000)
    spark.stop()
